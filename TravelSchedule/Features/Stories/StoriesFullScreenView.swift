...
struct StoriesFullScreenView: View {
                    ...
                    onLeftTap: {
                        Task { @MainActor in
                            if currentStoryIndex > 0 {
                                await switchToStory(at: currentStoryIndex - 1)
                            }
                        }
                    },
                    onRightTap: {
    ...
    private func setupInitialState() {
        timerManager.onProgressComplete = {
            Task { @MainActor in
                if currentStoryIndex < stories.count - 1 {
                    ...
                    timerManager.stop()
                    await delay(seconds: Constants.timerDelay)
                    isPresented = false
                }
            }
        }
        ...
        Task { @MainActor in
            await delay(seconds: Constants.timerDelay * 2)
    ...
    private func switchToStory(at index: Int) async {
        timerManager.stop()
        timerManager.progress = 0.0
        withAnimation {
